...
- **Node.js**: 22.21.1 LTS “Jod” – officially supported by Next.js 14.x (`>=18.17`), shadcn/ui, TanStack Query 5.x, Expo CLI 54.x. Node 24.11.0 LTS “Krypton” exists but Expo/Next.js have not yet declared formal support; revisit after vendor release notes update.
...
- **Portal**
  - Next.js `14.2.33` (App Router), React `18.3.1`, React DOM `18.3.1`.
  - UI kit: `shadcn/ui` (Radix UI `1.1.x`), data layer `@tanstack/react-query 5.90.7`.
...
- Pipelines: Ruff `0.7.x`, ESLint `9.x`, Pytest, Spectral `7.x`, Docker smoke tests, tag-triggered deploy.

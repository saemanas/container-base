...
3. **check_openapi** – installs Redocly CLI, lints `specs/001-lowcost-cicd-infra/contracts/openapi.yaml`, stores lint output/diff logs, and runs `docker run tufin/oasdiff:latest diff`/`breaking` to detect schema drifts and regressions. CI copies the base/head specs into `artifacts/ci/openapi_lint/`, executes the containerized comparisons (diff fails only on differences, breaking fails on ERR), and writes each stage log to `artifacts/ci/openapi_lint/{diff,breaking}.log`. Captured artifacts keep `{ts,opId,code,duration_ms}` metadata so auditors can trace OpenAPI compliance per the constitution’s single-source-of-truth requirement.

...
3. **OpenAPI Lint & Diff** – Lint the contract with `@redocly/cli@2.11.0` and compare against the base branch using `docker run tufin/oasdiff:latest diff`/`breaking`. CI copies both the base and head specs into `artifacts/ci/openapi_lint/`, runs the containerized `oasdiff diff --fail-on-diff`, and, when available, `oasdiff breaking --fail-on ERR`, capturing every log to `artifacts/ci/openapi_lint/{diff,breaking}.log`. Lint/diff outputs stay archived with `{ts,opId,code,duration_ms}` metadata so auditors can trace contract compliance plus PDPA rollback readiness.
